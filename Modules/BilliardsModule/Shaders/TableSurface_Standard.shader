Shader "metaphira/TableSurface"
{
   ...
   SubShader
   {
      ...
      float3 hueShift(float3 color, float shift)
      {
         ...
         float hue = atan2(oklab.z, oklab.y) + shift * M_PI * 2;
      ...
      float3 Unity_Saturation_float(float3 value, float saturation)
      {
         float luma = dot(value, float3(0.2126729, 0.7151522, 0.0721750));
         return luma.xxx + saturation.xxx * (value - luma.xxx);
      }

      void surf(Input IN, inout SurfaceOutputStandard o)
      {
         fixed4 sampleDiffuse = tex2D(_MainTex, IN.uv_MainTex);
         fixed4 sampleEmission = tex2D(_EmissionMap, IN.uv_MainTex);
         fixed4 sampleMetalic = tex2D(_Metalic, IN.uv_MainTex);

         fixed3 finalColor = lerp(sampleDiffuse.rgb, _Color * sampleDiffuse.rgb * 2.0, pow(sampleDiffuse.a, 0.1));

         #if defined(DETAIL_CLOTH)
            fixed4 detail = tex2D(_DetailCloth, IN.uv_MainTex * _DetailCloth_ST.xy + _DetailCloth_ST.wz)
               * unity_ColorSpaceDouble * _DetailClothBrightness;
            float clothMask = tex2D(_DetailClothMask, IN.uv_MainTex).r;
            finalColor = lerp(finalColor, finalColor * detail, clothMask * _MaskStrengthCloth);
            float3 cloth = hueShift(finalColor * clothMask, _ClothHue);
            cloth = Unity_Saturation_float(cloth, _ClothSaturation);
            finalColor = cloth + finalColor * (1 - clothMask);

            #if defined(DETAIL_OTHER)
               fixed4 otherDetail = tex2D(_DetailOther, IN.uv_MainTex * _DetailOther_ST.xy + _DetailOther_ST.wz)
                  * unity_ColorSpaceDouble * _DetailOtherBrightness;
               finalColor = lerp(finalColor, finalColor * otherDetail, (1 - clothMask) * _MaskStrengthOther);
            #endif
         #endif

         o.Albedo = finalColor;
         o.Metallic = sampleMetalic.r;
         o.Smoothness = sampleMetalic.a;
         o.Alpha = 1.0;
         ...
         o.Emission = sampleEmission.r * _EmissionColor * angle;
      }
      ENDCG
   }
